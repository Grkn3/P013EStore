...
namespace P013EStore.MVCUI.Controllers
{
    public class ProductsController : Controller
    {
        ...
        {
            var model = await _serviceProduct.GetAllAsync(p => p.IsActive && p.Name.Contains(q) || p.Description.Contains(q) ||p.Brand.Name.Contains(q) || p.Category.Name.Contains(q));
            return View(model);
        }
