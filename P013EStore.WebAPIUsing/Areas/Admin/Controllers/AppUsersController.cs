...
namespace P013EStore.WebAPIUsing.Areas.Admin.Controllers
{
    ...
    public class AppUsersController : Controller
    {
        ...
        private readonly string _apiAdres = "https://localhost:7136/api/AppUsers"; // api adresini web api projesini çalıştırdığımızda adres çubuğundan veya her hangi bir conroller a istek atarak Request URl kısmından veya api projesinde Properties altındaki launchSettings.json 

        public AppUsersController(HttpClient httpClient) 
        {
            _httpClient = httpClient; //_httpClient nesnesinin apiye ulaşması için api projesininde bu pprojeyle birlikte çalışıyor olması lazım
            //aynı anda 2 projeyi çalıştırabilmek için Solution a sağ tıklayıp açılan menüden configure startup projects diyip açılan ekrandan multiple alanında tıklayıp aynı anda başlatmak istediğimiz projeleri seçiyoruz!
        }
